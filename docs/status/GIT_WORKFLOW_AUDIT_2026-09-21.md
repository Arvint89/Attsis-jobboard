...
- **Uncommitted / untracked on `chore/jb-28-ci`:**
  ...
  - ⚠️ **`CLAUDE.md`, `ADR-001`, `.aider.conf.yml` are all untracked** — the branching decision, the aider config, and the project instructions have never been committed. If this branch is lost, so are they.
...
| Stale feature branches | `ADR-001:214` says "delete the 8 fully-merged stale branches" | All 8 still present locally + remote (`bugfix/jb-19-bamboohr-descriptions`, `bugfix/jb-25-geocoder`, `feature/jb-20-per-company-cap`, `feature/jb-22-distance-filter`, `feature/jb-23-search-row`, `feature/jb-26-company-map`, `feature/jb-3-board-sources`, `feature/jb-3b-ledc-scraper`) |
...
7. Delete the 8 stale branches + `develop`
...
- Every important decision (ADR-001, CLAUDE.md, aider config, CI workflow) is currently **uncommitted** on branch `chore/jb-28-ci`. Losing this branch loses the workflow decisions.
